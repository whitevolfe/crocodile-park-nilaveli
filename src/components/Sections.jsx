          ...
          <div className="relative mt-12 overflow-hidden rounded-3xl border border-primary/20 shadow-[var(--shadow-lux)]">
            {playing ? (
              <video
                ...
                poster={lagoonImage}
              >
                {/* Place the film at public/videos/crocodile-park-story.mp4 */}
...
export function Testimonials() {
  return (
    <section className="border-y border-primary/10 bg-navy/30 py-20 sm:py-28">
      <div className="mx-auto max-w-7xl px-5 sm:px-6">
        <Reveal>
          <SectionHeading
            eyebrow="Sample content"
            title="What Guests Could Say"
            subtitle="Placeholder testimonials shown as examples only — these are not real customer reviews and can be replaced with verified guest feedback."
          />
        </Reveal>
        <div className="mt-12 grid gap-6 md:grid-cols-3">
          {testimonials.map((t, i) => (
            <Reveal key={i} delay={i * 90}>
              <figure className="flex h-full flex-col gap-4 rounded-2xl border border-primary/15 bg-card/50 p-7">
                <Quote size={22} className="text-primary" />
                <blockquote className="text-sm leading-relaxed text-foreground/85">
                  {t.text}
                </blockquote>
                <figcaption className="mt-auto text-xs tracking-[0.2em] text-muted-foreground uppercase">
                  {t.name} — {t.origin}
                </figcaption>
              </figure>
            </Reveal>
